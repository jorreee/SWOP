...
public class TaskManInitFileChecker extends StreamTokenizer {
	...
	public void checkSystemFile() {
		...
		while (ttype == '-') {
			expectChar('-');
			dailyAvailabilityTime.add(new LocalTime[] {
					expectTimeField("startTime"), expectTimeField("endTime") });
		}
		...
		while (ttype == '-') {
			...
			resourcePrototypeDataList.add(new ResourcePrototypeCreationData(
					name, requirements, conflicts, availabilityIndex));
		}

		expectLabel("branches");
		while (ttype == '-') {
			expectChar('-');
			String fileName = expectStringField("file");
			branches.add(fileName);
		}
	}
	...
	public void checkFile() {
		...
		while (ttype == '-') {
			...
			concreteResourceDataList.add(new ConcreteResourceCreationData(name,
					typeIndex));
		}
		...
		while (ttype == '-') {
			...
			projectDataList.add(new ProjectCreationData(name, description,
		...
		while (ttype == '-') {
			...
			reservationDataList.add(new ReservationCreationData(resource, task,
					startTime, endTime));
		}
	...
	public Optional<LocalTime> getDailyAvailabilityStartByIndex(Integer index) {
		if (index == null)
			...
			return Optional.of(dailyAvailabilityTime.get(index)[0]);
	}
	...
	public Optional<LocalTime> getDailyAvailabilityEndByIndex(Integer index) {
		if (index == null)
			...
			return Optional.of(dailyAvailabilityTime.get(index)[1]);
	}
